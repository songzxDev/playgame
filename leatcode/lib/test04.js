...
const letterCombinations = function (digits) {
    let phone = new Map([['2', 'abc'], ['3', 'def'], ['4', 'ghi'], ['5', 'jkl'], ['6', 'mno'], ['7', 'pqrs'], ['8', 'tuv'], ['9', 'wxyz']]);
    let backtrack = (comb, next) => {
        if (comb.length === 0) {
            comb = next.split('');
        } else {
            let tmp = [];
            for (let curr of comb) {
                for (let s of next.split('')) {
                    tmp.push(curr + s);
                }
            }
            comb = tmp;
    ...
    let res = [];
    for (let dg of digits) {
        if (phone.has(dg)) {
            res = backtrack(res, phone.get(dg));
        }
    }
    return res;
};
